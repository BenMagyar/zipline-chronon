...
class IcebergTest extends SparkTestBase with Matchers {
  ...
  it should "return only partition columns from the current Iceberg spec" in {
    val tableName = "default.iceberg_driver_current_partition_spec_test"
    spark.sql(s"DROP TABLE IF EXISTS $tableName")
    ...
    try {
      ...
      Iceberg.partitionColumnNames(tableName) shouldBe Seq("ds")

      spark.sql(s"ALTER TABLE $tableName DROP PARTITION FIELD ds")

      Iceberg.partitionColumnNames(tableName) shouldBe Seq.empty
    } finally {
      spark.sql(s"DROP TABLE IF EXISTS $tableName")
    }
  }
